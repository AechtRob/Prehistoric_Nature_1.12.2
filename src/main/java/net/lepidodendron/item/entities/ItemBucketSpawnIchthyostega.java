...
public class ItemBucketSpawnIchthyostega extends ElementsLepidodendronMod.ModElement {
	...
	public void registerModels(ModelRegistryEvent event) {
		ModelLoader.setCustomModelResourceLocation(block, 0, new ModelResourceLocation("lepidodendron:entities/ichthyostega_spawn_bucket", "inventory"));
	}
			...
			//Entity MobSpawn = null;
			{
				...
				else
				{
					...
					else
					{
						...
						if (!(worldIn.isRemote) && (worldIn.getBlockState(posIn.up()).getMaterial() == Material.WATER)
							&& (worldIn.getBlockState(posIn.down()).getBlockFaceShape(worldIn, posIn.down(), EnumFacing.UP) == BlockFaceShape.SOLID)){
							worldIn.setBlockState(posIn, BlockAmphibianSpawnIchthyostegaPlaceable.block.getDefaultState());
						}
					}
