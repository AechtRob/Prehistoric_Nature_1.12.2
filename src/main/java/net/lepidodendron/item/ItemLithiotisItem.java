...
import net.lepidodendron.block.BlockLithiotis;
...
public class ItemLithiotisItem extends ElementsLepidodendronMod.ModElement {
	@GameRegistry.ObjectHolder("lepidodendron:lithiotis_item")
	public static final Item block = null;

	public ItemLithiotisItem(ElementsLepidodendronMod instance) {
		super(instance, LepidodendronSorter.lithiotis_item);
	}
	...
	public void registerModels(ModelRegistryEvent event) {
		ModelLoader.setCustomModelResourceLocation(block, 0, new ModelResourceLocation("lepidodendron:lithiotis_item", "inventory"));
	}

	public static class ItemCustom extends Item {
		public ItemCustom() {
			setTranslationKey("pf_lithiotis_item");
			setRegistryName("lithiotis_item");
			...
			if (raytraceresult == null) {
				...
				if (raytraceresult.typeOfHit == RayTraceResult.Type.BLOCK) {
					...
					if (canSurviveAt(worldIn, blockpos1) && (iblockstate.getMaterial() == Material.WATER && ((Integer) iblockstate.getValue(BlockLiquid.LEVEL)).intValue() == 0 && worldIn.isAirBlock(blockpos1))) {
						...
						worldIn.setBlockState(blockpos1, BlockLithiotis.block.getDefaultState());
						if (net.minecraftforge.event.ForgeEventFactory.onPlayerBlockPlace(playerIn, blocksnapshot, net.minecraft.util.EnumFacing.UP, handIn).isCanceled()) {
							blocksnapshot.restore(true, false);
							return new ActionResult<ItemStack>(EnumActionResult.FAIL, itemstack);
						}

						worldIn.setBlockState(blockpos1, BlockLithiotis.block.getDefaultState(), 11);
		...
		public void addInformation(ItemStack stack, World player, List<String> tooltip, ITooltipFlag advanced) {
			if (LepidodendronConfig.showTooltips) {
				tooltip.add("Type: Reef-forming marine bivalve");
