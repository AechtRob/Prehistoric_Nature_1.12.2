                ...
                while (zct <= layerRadius) {
                    ...
                    if (Math.random() < reefChance) {
                        if (((worldIn.getBlockState(pos.down()).getMaterial() == Material.SAND)
                            ...
                            worldIn.setBlockState(pos, BlockCoral.block.getDefaultState().withProperty(BlockCoral.FACING, facing));
                            ...
                            if ((Math.random() > 0.65) && (position.getY() + layer + 1) < worldIn.getSeaLevel()) {
                                ...
                                if (ii == 2) {facing = EnumFacing.EAST;}
                                if (ii == 3) {facing = EnumFacing.WEST;}
                                worldIn.setBlockState(pos.up(), BlockCoral.block.getDefaultState().withProperty(BlockCoral.FACING, facing));
                                if ((Math.random() > 0.65) && (position.getY() + layer + 2) < worldIn.getSeaLevel()) {
                                    ...
                                    if (ii == 2) {facing = EnumFacing.EAST;}
                                    if (ii == 3) {facing = EnumFacing.WEST;}
                                    worldIn.setBlockState(pos.up(2), BlockCoral.block.getDefaultState().withProperty(BlockCoral.FACING, facing));
                                }
                            }
                        }
                    }
