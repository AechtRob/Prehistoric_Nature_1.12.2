        ...
        for (int i = 0; i < 48; ++i)
        {
            ...
            )
            {
                if (rand.nextInt(4) == 0) {
                    ...
                    if (worldIn.isAirBlock(blockpos.up())) {worldIn.setBlockState(blockpos.up(), getleafstate(leafState), 2);}
                    if (worldIn.isAirBlock(blockpos.north())) {worldIn.setBlockState(blockpos.north(), getleafstate(leafState), 2);}
                    if (worldIn.isAirBlock(blockpos.south())) {worldIn.setBlockState(blockpos.south(), getleafstate(leafState), 2);}
                    if (worldIn.isAirBlock(blockpos.east())) {worldIn.setBlockState(blockpos.east(), getleafstate(leafState), 2);}
                    if (worldIn.isAirBlock(blockpos.west())) {worldIn.setBlockState(blockpos.west(), getleafstate(leafState), 2);}
                }
                else {
