...
public class StructureSpawnMirabilis extends ElementsLepidodendronMod.ModElement {
	public StructureSpawnMirabilis(ElementsLepidodendronMod instance) {
		super(instance, 48);
	}
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (shouldGenerateInDimension(dimID, LepidodendronConfig.dimAraucariaMirabilis))
			dimensionCriteria = true;
		if (!LepidodendronConfig.genAraucariaMirabilis && !LepidodendronConfig.genAllPlants)
			dimensionCriteria = false;
		if (!dimensionCriteria)
			return;
		boolean biomeCriteria = false;
		Biome biome = world.getBiome(new BlockPos(i2, world.getSeaLevel(), k2));
		if ((!matchBiome(biome, LepidodendronConfig.genGlobalBlacklist)) && (!matchBiome(biome, LepidodendronConfig.genAraucariaMirabilisBlacklistBiomes))) {
		...
		if (matchBiome(biome, LepidodendronConfig.genAraucariaMirabilisOverrideBiomes))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;

		int GenChance = 40000;
		double GenMultiplier = LepidodendronConfig.multiplierAraucariaMirabilis;
		...
		if ((random.nextInt(1000000) + 1) <= GenChance) {
			...
			for (int a = 0; a < count; a++) {
				...
				int maxheight = LepidodendronConfig.maxheightAraucariaMirabilis;
				int minheight = LepidodendronConfig.minheightAraucariaMirabilis;
...

				{
					...
					ProcedureWorldGenMirabilis.executeProcedure($_dependencies);
				}
