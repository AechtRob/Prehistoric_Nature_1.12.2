...
import net.lepidodendron.procedure.ProcedureWorldGenMonkeyPuzzleNoCheck;
...
public class StructureSpawnMonkeyPuzzle extends ElementsLepidodendronMod.ModElement {
	public StructureSpawnMonkeyPuzzle(ElementsLepidodendronMod instance) {
		super(instance, 48);
	}
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (shouldGenerateInDimension(dimID, LepidodendronConfig.dimAraucariaAraucana))
			dimensionCriteria = true;
		if (!LepidodendronConfig.genAraucariaAraucana && !LepidodendronConfig.genAllPlants)
		...
		if ((!matchBiome(biome, LepidodendronConfig.genGlobalBlacklist)) && (!matchBiome(biome, LepidodendronConfig.genAraucariaAraucanaBlacklistBiomes))) {
		...
		if (matchBiome(biome, LepidodendronConfig.genAraucariaAraucanaOverrideBiomes))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;
			
		int GenChance = 15000;
		double GenMultiplier = LepidodendronConfig.multiplierAraucariaAraucana;
		...
		if ((random.nextInt(1000000) + 1) <= GenChance) {
			...
			for (int a = 0; a < count; a++) {
				...
				int maxheight = LepidodendronConfig.maxheightAraucariaAraucana;
				int minheight = LepidodendronConfig.minheightAraucariaAraucana;
