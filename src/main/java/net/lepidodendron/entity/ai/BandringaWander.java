...
public class BandringaWander extends AnimationAINoAnimation<EntityPrehistoricFloraBandringa> {
    ...
    public boolean shouldExecute() {
        ...
        if (this.PrehistoricFloraBandringa.getRNG().nextFloat() < 0.5F) {
            ...
            if (!this.PrehistoricFloraBandringa.getNavigator().noPath() && !isDirectPathBetweenPoints(this.PrehistoricFloraBandringa, this.PrehistoricFloraBandringa.getPositionVector(), new Vec3d(path.getFinalPathPoint().x, path.getFinalPathPoint().y, path.getFinalPathPoint().z)) || path != null && path.getFinalPathPoint() != null && this.PrehistoricFloraBandringa.getDistanceSq(path.getFinalPathPoint().x, path.getFinalPathPoint().y + 0.5, path.getFinalPathPoint().z + 0.5) < 3) {
