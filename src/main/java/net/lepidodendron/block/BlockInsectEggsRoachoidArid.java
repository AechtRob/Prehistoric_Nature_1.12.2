...
public class BlockInsectEggsRoachoidArid extends ElementsLepidodendronMod.ModElement {
	...
	public static class BlockCustom extends BlockInsectEggs {
		...
		public void updateTick(World worldIn, BlockPos pos, IBlockState state, Random rand)
		{
			...
			if (!(worldIn.isRemote)) {
				EntityPrehistoricFloraAgeableBase.summon(worldIn, EntityList.getKey(EntityPrehistoricFloraRoachoidArid.class).toString(), "", (double)pos.getX() + 0.5D, (double)pos.getY() + 0.5D, (double)pos.getZ() + 0.5D);
			}
		...
		public void addInformation(ItemStack stack, World player, List<String> tooltip, ITooltipFlag advanced) {
			if (LepidodendronConfig.showTooltips) {
				tooltip.add("Type: " + EntityPrehistoricFloraRoachoidArid.getHabitat() + " Insect");
				tooltip.add("Periods: " + EntityPrehistoricFloraRoachoidArid.getPeriod());
