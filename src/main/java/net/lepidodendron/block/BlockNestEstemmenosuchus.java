...
public class BlockNestEstemmenosuchus extends ElementsLepidodendronMod.ModElement {
	@GameRegistry.ObjectHolder("lepidodendron:nest_estemmenosuchus")
	public static final Block block = null;
	public BlockNestEstemmenosuchus(ElementsLepidodendronMod instance) {
		super(instance, LepidodendronSorter.nest_estemmenosuchus);
	}

	@Override
	public void initElements() {
		elements.blocks.add(() -> new BlockCustom().setRegistryName("nest_estemmenosuchus"));
		elements.items.add(() -> new ItemBlock(block).setRegistryName(block.getRegistryName()));
	}

	@Override
	public void init(FMLInitializationEvent event) {
		GameRegistry.registerTileEntity(BlockNestEstemmenosuchus.TileEntityCustom.class, "lepidodendron:tileentitynest_estemmenosuchus");
	}
	...
	public void registerModels(ModelRegistryEvent event) {
		ModelLoader.setCustomModelResourceLocation(Item.getItemFromBlock(block), 0,
				new ModelResourceLocation("lepidodendron:nest_estemmenosuchus", "inventory"));
	}
	public static class BlockCustom extends Block implements IShearable {
			...
			if (!worldIn.isRemote) {
				...
				if (stack.getItem() == Items.SHEARS) {
					...
					if (!playerIn.capabilities.isCreativeMode) {
						List<EntityPrehistoricFloraEstemmenosuchus> Estemmenosuchus = worldIn.getEntitiesWithinAABB(EntityPrehistoricFloraEstemmenosuchus.class, new AxisAlignedBB(pos.add(-16, -8, -16), pos.add(16, 8, 16)));
						for (EntityPrehistoricFloraEstemmenosuchus currentEstemmenosuchus : Estemmenosuchus) {
							if (currentEstemmenosuchus.isPFAdult()) {
								currentEstemmenosuchus.setAttackTarget(playerIn);
								currentEstemmenosuchus.setOneHit(true);
							}
						}
					}
				}
		...
		public boolean onBlockActivated(World world, BlockPos pos, IBlockState state, EntityPlayer player, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ)
		{
			...
			if ((!player.capabilities.allowEdit) || (!player.getHeldItemMainhand().isEmpty()) || eggRenderType.equals(""))
			{
				...
				if (!((hand != player.getActiveHand()) && (hand == EnumHand.MAIN_HAND))) {
					ItemStack stackSeed = new ItemStack(BlockEggsEstemmenosuchus.block, (int) (1));
					...
					if (!player.capabilities.isCreativeMode) {
						List<EntityPrehistoricFloraEstemmenosuchus> Estemmenosuchus = world.getEntitiesWithinAABB(EntityPrehistoricFloraEstemmenosuchus.class, new AxisAlignedBB(pos.add(-16, -8, -16), pos.add(16, 8, 16)));
						for (EntityPrehistoricFloraEstemmenosuchus currentEstemmenosuchus : Estemmenosuchus) {
							if (currentEstemmenosuchus.isPFAdult()) {
								currentEstemmenosuchus.setAttackTarget(player);
								currentEstemmenosuchus.setOneHit(true);
							}
						}
					}
		...
		public void SpawnEggs(World world, BlockPos pos) {
			...
			if (!eggRenderType.equals("")) {
				Block blockSpawn = BlockEggsEstemmenosuchus.block;
		...
		public TileEntity createTileEntity(World world, IBlockState state) {
			return new BlockNestEstemmenosuchus.TileEntityCustom();
		}

		public BlockNestEstemmenosuchus.TileEntityCustom createNewTileEntity(World worldIn, int meta) {
			return new BlockNestEstemmenosuchus.TileEntityCustom();
		}
