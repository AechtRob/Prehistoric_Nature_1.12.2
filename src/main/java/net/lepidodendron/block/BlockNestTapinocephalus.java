...
public class BlockNestTapinocephalus extends ElementsLepidodendronMod.ModElement {
	@GameRegistry.ObjectHolder("lepidodendron:nest_tapinocephalus")
	public static final Block block = null;
	public BlockNestTapinocephalus(ElementsLepidodendronMod instance) {
		super(instance, LepidodendronSorter.nest_tapinocephalus);
	}

	@Override
	public void initElements() {
		elements.blocks.add(() -> new BlockCustom().setRegistryName("nest_tapinocephalus"));
		elements.items.add(() -> new ItemBlock(block).setRegistryName(block.getRegistryName()));
	}

	@Override
	public void init(FMLInitializationEvent event) {
		GameRegistry.registerTileEntity(BlockNestTapinocephalus.TileEntityCustom.class, "lepidodendron:tileentitynest_tapinocephalus");
	}
	...
	public void registerModels(ModelRegistryEvent event) {
		ModelLoader.setCustomModelResourceLocation(Item.getItemFromBlock(block), 0,
				new ModelResourceLocation("lepidodendron:nest_tapinocephalus", "inventory"));
	}
	public static class BlockCustom extends Block implements IShearable {
			...
			if (!worldIn.isRemote) {
				...
				if (stack.getItem() == Items.SHEARS) {
					...
					if (!playerIn.capabilities.isCreativeMode) {
						List<EntityPrehistoricFloraTapinocephalus> Tapinocephalus = worldIn.getEntitiesWithinAABB(EntityPrehistoricFloraTapinocephalus.class, new AxisAlignedBB(pos.add(-16, -8, -16), pos.add(16, 8, 16)));
						for (EntityPrehistoricFloraTapinocephalus currentTapinocephalus : Tapinocephalus) {
							if (currentTapinocephalus.isPFAdult()) {
								currentTapinocephalus.setAttackTarget(playerIn);
								currentTapinocephalus.setOneHit(true);
							}
						}
					}
				}
		...
		public boolean onBlockActivated(World world, BlockPos pos, IBlockState state, EntityPlayer player, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ)
		{
			...
			if ((!player.capabilities.allowEdit) || (!player.getHeldItemMainhand().isEmpty()) || eggRenderType.equals(""))
			{
				...
				if (!((hand != player.getActiveHand()) && (hand == EnumHand.MAIN_HAND))) {
					ItemStack stackSeed = new ItemStack(BlockEggsTapinocephalus.block, (int) (1));
					...
					if (!player.capabilities.isCreativeMode) {
						List<EntityPrehistoricFloraTapinocephalus> Tapinocephalus = world.getEntitiesWithinAABB(EntityPrehistoricFloraTapinocephalus.class, new AxisAlignedBB(pos.add(-16, -8, -16), pos.add(16, 8, 16)));
						for (EntityPrehistoricFloraTapinocephalus currentTapinocephalus : Tapinocephalus) {
							if (currentTapinocephalus.isPFAdult()) {
								currentTapinocephalus.setAttackTarget(player);
								currentTapinocephalus.setOneHit(true);
							}
						}
					}
		...
		public void SpawnEggs(World world, BlockPos pos) {
			...
			if (!eggRenderType.equals("")) {
				Block blockSpawn = BlockEggsTapinocephalus.block;
		...
		public TileEntity createTileEntity(World world, IBlockState state) {
			return new BlockNestTapinocephalus.TileEntityCustom();
		}

		public BlockNestTapinocephalus.TileEntityCustom createNewTileEntity(World worldIn, int meta) {
			return new BlockNestTapinocephalus.TileEntityCustom();
		}
