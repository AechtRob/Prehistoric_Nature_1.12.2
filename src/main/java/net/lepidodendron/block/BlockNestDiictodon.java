...
public class BlockNestDiictodon extends ElementsLepidodendronMod.ModElement {
	@GameRegistry.ObjectHolder("lepidodendron:nest_diictodon")
	public static final Block block = null;
	public BlockNestDiictodon(ElementsLepidodendronMod instance) {
		super(instance, LepidodendronSorter.nest_diictodon);
	}

	@Override
	public void initElements() {
		elements.blocks.add(() -> new BlockCustom().setRegistryName("nest_diictodon"));
		elements.items.add(() -> new ItemBlock(block).setRegistryName(block.getRegistryName()));
	}

	@Override
	public void init(FMLInitializationEvent event) {
		GameRegistry.registerTileEntity(BlockNestDiictodon.TileEntityCustom.class, "lepidodendron:tileentitynest_diictodon");
	}
	...
	public void registerModels(ModelRegistryEvent event) {
		ModelLoader.setCustomModelResourceLocation(Item.getItemFromBlock(block), 0,
				new ModelResourceLocation("lepidodendron:nest_diictodon", "inventory"));
	}
	public static class BlockCustom extends Block implements IShearable {
			...
			if (!worldIn.isRemote) {
				...
				if (stack.getItem() == Items.SHEARS) {
					...
					if (!playerIn.capabilities.isCreativeMode) {
						List<EntityPrehistoricFloraDiictodon> Diictodon = worldIn.getEntitiesWithinAABB(EntityPrehistoricFloraDiictodon.class, new AxisAlignedBB(pos.add(-16, -8, -16), pos.add(16, 8, 16)));
						for (EntityPrehistoricFloraDiictodon currentDiictodon : Diictodon) {
							if (currentDiictodon.isPFAdult()) {
								currentDiictodon.setAttackTarget(playerIn);
								currentDiictodon.setOneHit(true);
							}
						}
					}
				}
		...
		public boolean onBlockActivated(World world, BlockPos pos, IBlockState state, EntityPlayer player, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ)
		{
			...
			if ((!player.capabilities.allowEdit) || (!player.getHeldItemMainhand().isEmpty()) || eggRenderType.equals(""))
			{
				...
				if (!((hand != player.getActiveHand()) && (hand == EnumHand.MAIN_HAND))) {
					ItemStack stackSeed = new ItemStack(BlockEggsDiictodon.block, (int) (1));
					...
					if (!player.capabilities.isCreativeMode) {
						List<EntityPrehistoricFloraDiictodon> Diictodon = world.getEntitiesWithinAABB(EntityPrehistoricFloraDiictodon.class, new AxisAlignedBB(pos.add(-16, -8, -16), pos.add(16, 8, 16)));
						for (EntityPrehistoricFloraDiictodon currentDiictodon : Diictodon) {
							if (currentDiictodon.isPFAdult()) {
								currentDiictodon.setAttackTarget(player);
								currentDiictodon.setOneHit(true);
							}
						}
					}
		...
		public void SpawnEggs(World world, BlockPos pos) {
			...
			if (!eggRenderType.equals("")) {
				Block blockSpawn = BlockEggsDiictodon.block;
		...
		public TileEntity createTileEntity(World world, IBlockState state) {
			return new BlockNestDiictodon.TileEntityCustom();
		}

		public BlockNestDiictodon.TileEntityCustom createNewTileEntity(World worldIn, int meta) {
			return new BlockNestDiictodon.TileEntityCustom();
		}
