			...
			if (!worldIn.isRemote) {
				...
				if (stack.getItem() == Items.SHEARS) {
					...
					if (!playerIn.capabilities.isCreativeMode) {
						List<EntityPrehistoricFloraCotylorhynchus> Cotylorhynchus = worldIn.getEntitiesWithinAABB(EntityPrehistoricFloraCotylorhynchus.class, new AxisAlignedBB(pos.add(-16, -8, -16), pos.add(16, 8, 16)));
						for (EntityPrehistoricFloraCotylorhynchus currentCotylorhynchus : Cotylorhynchus) {
							if (currentCotylorhynchus.isPFAdult()) {
								currentCotylorhynchus.setAttackTarget(playerIn);
		...
		public boolean onBlockActivated(World world, BlockPos pos, IBlockState state, EntityPlayer player, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ)
		{
			...
			if ((!player.capabilities.allowEdit) || (!player.getHeldItemMainhand().isEmpty()) || eggRenderType.equals(""))
			{
				...
				if (!((hand != player.getActiveHand()) && (hand == EnumHand.MAIN_HAND))) {
					ItemStack stackSeed = new ItemStack(BlockEggsCotylorhynchus.block, (int) (1));
