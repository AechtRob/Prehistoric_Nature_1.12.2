	...
	public static class BlockCustom extends BlockMobSpawn {
		...
		public void updateTick(World worldIn, BlockPos pos, IBlockState state, Random rand)
		{
			...
			if (worldIn.getBlockState(pos).getBlock() == this && !(worldIn.isRemote)) {
				...
				Entity entity1 = ItemMonsterPlacer.spawnCreature(worldIn, EntityList.getKey(EntityPrehistoricFloraKokomopterus.class), (double)pos.getX() + 0.5D, (double)pos.getY() + 0.5D, (double)pos.getZ() + 0.5D);
				EntityPrehistoricFloraAgeableBase ee1 = (EntityPrehistoricFloraAgeableBase) entity1;
				ee1.setAgeTicks(1);
				Entity entity2 = null;
				if (Math.random() > 0.75) {
					entity2 = ItemMonsterPlacer.spawnCreature(worldIn, EntityList.getKey(EntityPrehistoricFloraKokomopterus.class), (double)pos.getX() + 0.5D, (double)pos.getY() + 0.5D, (double)pos.getZ() + 0.5D);
