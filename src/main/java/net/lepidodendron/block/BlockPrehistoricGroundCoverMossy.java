...
public class BlockPrehistoricGroundCoverMossy extends ElementsLepidodendronMod.ModElement {
	@GameRegistry.ObjectHolder("lepidodendron:mossy_prehistoric_ground_cover")
	public static final Block block = null;
	public BlockPrehistoricGroundCoverMossy(ElementsLepidodendronMod instance) {
		super(instance, 526);
	}
		                ...
		                if (worldIn.getLightFromNeighbors(pos.up()) >= 9)
		                {
		                    for (int i = 0; i < 4; ++i)
		                    {
		                        ...
		                        if (iblockstate1.getBlock() == Blocks.STONE && worldIn.getLightFromNeighbors(blockpos.up()) >= 4 && iblockstate.getLightOpacity(worldIn, pos.up()) <= 2)
		                        {
									...
									if (waterCriteria)
		                            	worldIn.setBlockState(blockpos, BlockPrehistoricGroundCoverMossy.block.getDefaultState());
		                        }
		                    }
	                    }
	                }
